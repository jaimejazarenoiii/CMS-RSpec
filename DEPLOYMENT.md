...
sudo systemctl <start|stop|restart|status> cms_rspec-puma.service
sudo systemctl <start|stop|restart|status> cms_rspec-sidekiq.service
```

Note that the `deployer` user has permission to restart `cms_rspec-puma.service` and `cms_rspec-sidekiq.service`, but nothing else. All other `systemctl` commands require `root`.
...
```
/home/deployer/apps/cms_rspec/shared/.env.production
```

After making changes to this file, be sure to restart the application:

```
sudo systemctl restart cms_rspec-puma.service
sudo systemctl restart cms_rspec-sidekiq.service
```
