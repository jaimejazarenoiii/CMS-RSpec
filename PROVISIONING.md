# How to provision a new environment for cms_rspec
...
* Enter webroot path: `/home/deployer/apps/cms_rspec/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/example.com/fullchain.pem /etc/ssl/cms_rspec.crt
ln -s /etc/letsencrypt/live/example.com/privkey.pem /etc/ssl/cms_rspec.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/cms_rspec/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
7 6,18 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/cms_rspec/shared/public/
```
